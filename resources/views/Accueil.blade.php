         ...
         <div class="image-slider-track-container">
            <ul class="image-slider-track">
               ...
               <li class="image-slide">
                  @php
                     $randomImage = $article->images->isNotEmpty() ? $article->images->random()->path : '';
                  @endphp
                  @if ($randomImage)
                  <img src="{{ asset($randomImage) }}" alt="Image {{ $article->title }}">
                  @endif
                  <div class="news-details">
                     <div class="news-header">
                        <span class="news-date">{{ \Carbon\Carbon::parse($article->publication_date)->translatedFormat('d F Y') }}</span>
                        <span class="media-release">Media Release</span>
                     </div>
                     <div class="news-content">
                        <h2>{{ $article->title }}</h2>
                        <a href="{{ route('voir-plus-news', $article->id) }}">Voir plus</a>
                     </div>
                  </div>
               </li>
               @endforeach
